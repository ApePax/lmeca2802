...
class Contact_Manager:
    def __init__(self, nSensors):
        ...
        Contact_PxF: Memory of the contact points (accessed through ixF)
        Previous_PxF: Memory of the positions of the sensors at the previous itteration
        InContact: Binary memory set to 1 if the sensor is in contact 
        """
        self.nSensors = nSensors
        self.Contact_PxF  = np.full(nSensors,None)
        self.Previous_PxF = np.full(nSensors,None)
        self.InContact    = np.zeros(nSensors)
    ...
    def update_contact(self,ixF, PxF):
        ...
        PxF = PxF[0:4]
        # Initialisation
        if(self.Previous_PxF[ixF] == None):
            self.Previous_PxF[ixF] = PxF
        # Detection of the penetration
        if(PxF[3] <= 0):
            # Update the contact point
            if(self.InContact[ixF] == 0):
                # Linear interpolation of the contact point bewteen the previous and current point of contact
                ratio = self.Previous_PxF[3]/(self.Previous_PxF[3]-PxF[3])
                self.Contact_PxF[ixF][1] = self.Previous_PxF + ratio * (PxF[1] - self.Previous_PxF[1])
                self.Contact_PxF[ixF][2] = self.Previous_PxF + ratio * (PxF[2] - self.Previous_PxF[2])
                self.Contact_PxF[ixF][3] = 0.0
                # Report Contact
                self.InContact[ixF] = 1
            else:
                pass
        # Detection of no-contact
        else:
    ...
    def update_slip_contact(self,ixF,PxF, delta_slip_x,delta_slip_y, slip):
        if slip == 1 and self.InContact[ixF] == 1:
            self.Contact_PxF[ixF] -= np.array([0.0,delta_slip_x,delta_slip_y,0.0])
        return
    ...
    def compute_penetrations(self, ixF, PxF, VxF):
        ...
        if(self.InContact[ixF] == 1):
            deltas = PxF - self.Contact_PxF[ixF]
            deltas_dot = VxF
...
class HuntCrossleyHertz:
    def __init__(self, k, n, d):
        ...
        d (float): Damping coefficient (𝜆𝑛 = 3/2 𝛼 k => d = 3/2 𝛼)
        From Xing Chen:
        𝜆𝑛 = 3/2 𝛼 k => d = 3/2 𝛼
        𝛼 = 0.2
        k,n,d = [5*10**4, 3/2 , 0.3]
        """
        self.k = k
        self.n = n
        self.d = d
...
class ViscoelasticCoulombModel:
    ...
    def compute_tangential_force(self, F_n, delta_x, delta_x_dot, delta_y, delta_y_dot):
        ...
        delta_t (float): Tangential displacement (m)
        delta_t_dot (float): Tangential velocity (m/s)
        ...
        # Maximum lateral penetrations without slip
        delta_no_slip_x = delta_x
        delta_no_slip_y = delta_y
        slip = 0
        # Apply saturation
        if F_total > F_max:
            ...
            Fx *= scaling_factor
            Fy *= scaling_factor
            # Inverse model for saturated force
            delta_no_slip_x = -(Fx + self.d * delta_x_dot)/self.k 
            delta_no_slip_y = -(Fy + self.d * delta_x_dot)/self.k 
            slip = 1
    ...
    def compute_normal_force(self, delta, delta_dot):
        ...
        Parameters:
        delta (float): normal displacement, penetration (m)
        delta_t_dot (float): normal velocity (m/s)

        Returns:
        float: Friction force (N)
        """
        return -self.k * delta - self.d * delta_dot
...
def user_ExtForces(PxF, RxF, VxF, OMxF, AxF, OMPxF, mbs_data, tsim, ixF):
    ...
    # Compute the penetrations and penetration speeds
    deltas, deltas_dot = cm.compute_penetrations(ixF, PxF)

    # Normal force computation
    Model = 1 # [ 0 : Hunt-Crossley-Hertz ; 1 : Viscoelastic Coulomb ]
    if(Model == 0):
    ...
    # Compute the tangential forces
    Fx, Fy, delta_slip_x, delta_slip_y, slip = vc_model.compute_tangential_force(Fz,deltas[0],deltas_dot[0],deltas[1],deltas_dot[1])

    # Update the contact points in case of slip
